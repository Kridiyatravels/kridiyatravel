-- Fix: the corporate form's payment field sends values like "Bank transfer",
-- but corporate_accounts.payment_terms only allows payment_before_booking /
-- credit_approved / monthly_billing. Map to a valid value (default = the
-- business rule "payment before booking"). Only the v_payment_terms line
-- changed vs the previous version.
CREATE OR REPLACE FUNCTION public.convert_corporate_enquiry_to_booking(p_enquiry_id uuid, p_corporate_account_id uuid DEFAULT NULL::uuid, p_title text DEFAULT NULL::text, p_service_type booking_service_type DEFAULT NULL::booking_service_type, p_route_or_destination text DEFAULT NULL::text, p_travel_start date DEFAULT NULL::date, p_travel_end date DEFAULT NULL::date, p_notes text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
