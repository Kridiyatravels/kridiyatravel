-- Legacy bookings without a positive sale total cannot carry paid/partial labels.
...
begin
  ...
  if coalesce(sale_total,0)<=0 and new.payment_status in ('paid','partially_paid') then new.payment_status:='not_requested'; return new; end if;
  select coalesce(sum(p.amount),0) into received_total from public.payments p where p.booking_id=new.id and p.payment_direction='customer_in' and p.status in ('received','chargeback_won');
  ...
  select exists(select 1 from public.payments p where p.booking_id=new.id and p.status in ('refund_pending','refund_approved')), exists(select 1 from public.payments p where p.booking_id=new.id and p.status='refunded') into refund_open,refund_completed;
  ...
  if new.payment_status='partially_paid' and (received_total<=0 or received_total>=sale_total) then raise exception 'Partially paid status requires a positive outstanding customer balance'; end if;
