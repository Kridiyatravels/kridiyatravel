...
-- Adds a public.enquiries table that every enquiry form (guest or signed-in)
-- writes to, plus a staff-only public.enquiry_notes table for internal notes.
-- Run this in the same Supabase project as kridiya_phase1_schema.sql.
...
-- Anyone (guest or signed-in) may create an enquiry for themselves. They
-- cannot set status/other people's user_id at insert time.
...
-- Internal notes are staff-only in both directions - a customer never sees
-- notes about their own enquiry, even though they can see the enquiry row.
