-- Kridiya Travel - staff activity log (phase 6)
-- Wires up the (previously unused) public.audit_events table so
-- owners/admins can see what every staff member has been doing.
-- Regular "staff"/"support" can act (insert events) but only
-- "owner"/"admin" can read the log back.
