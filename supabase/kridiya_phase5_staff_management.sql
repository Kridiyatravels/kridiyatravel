-- Kridiya Travel - self-service staff management (phase 5)
-- Lets an existing staff member grant/revoke/list staff access from the
-- admin UI, without needing SQL run for them each time. auth.users isn't
-- directly queryable by clients, so these are security definer functions
-- that do the privileged email lookup internally, gated by is_staff().
