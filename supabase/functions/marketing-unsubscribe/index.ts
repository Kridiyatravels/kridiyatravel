...
Deno.serve(async (request: Request) => {
  ...
  const ipHash = await hash(clientAddress(request), signingSecret);
  const emailHash = await hash(email, signingSecret);
  const { data: admitted, error: admissionError } = await admin.rpc("admit_marketing_unsubscribe", {
    p_ip_hash: ipHash,
    p_email_hash: emailHash,
  });
  if (admissionError) {
    console.error("marketing-unsubscribe: rate-limit admission failed", admissionError.message);
    return json(origin, 503, { error: "Confirmation email is temporarily unavailable" });
  }
  // Use the same response as a successful request so callers cannot use this
  // endpoint to learn whether an address is subscribed or rate limited.
  if (admitted !== true) return json(origin, 202, { ok: true, pending_confirmation: true });
